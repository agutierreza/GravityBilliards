...
protected:
    ...
    ParticleSimulator<EulerIntegrator, CircleCollisionDetector, InelasticCollisionResolver> createParticleSimulator() {
...
TEST_F(SimulationRunnerTest, AccumulatorLogicExactlyTwoSteps) {
    auto loop = createParticleSimulator();
    ...
    EXPECT_NEAR(runner.getParticleSimulator().getCurrentTime(), 2.0, 1e-9);
}
...
TEST_F(SimulationRunnerTest, AccumulatorLogicPartialStep) {
    ...
    EXPECT_NEAR(runner.getParticleSimulator().getCurrentTime(), 3.0, 1e-9);
}

TEST_F(SimulationRunnerTest, ForceInputAppliedToParticleSimulator) {
    auto loop = createParticleSimulator();
    SimulationRunner<EulerIntegrator, CircleCollisionDetector, InelasticCollisionResolver> runner(std::move(loop), mockInput);
    ...
    EXPECT_NEAR(runner.getParticleSimulator().getVelocity().x, 10.0, 1e-9);
    EXPECT_NEAR(runner.getParticleSimulator().getVelocity().y, 0.0, 1e-9);
    
    // Another step of 1.0. Force applies again, V2 becomes (20,0). 
    runner.update(1.0);
    
    EXPECT_EQ(mockInput->pollCount, 2);
    EXPECT_NEAR(runner.getParticleSimulator().getVelocity().x, 20.0, 1e-9);
    EXPECT_NEAR(runner.getParticleSimulator().getVelocity().y, 0.0, 1e-9);
}
